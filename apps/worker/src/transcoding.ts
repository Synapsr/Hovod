...
export async function transcodeRendition(
  ...
  await runFfmpeg([
    '-y',
    '-i', sourcePath,
    // Output option (after -i): limits the x264 encoder threads; before -i it would only
    // configure the decoder and the hardware-adaptive thread budget would be inert.
