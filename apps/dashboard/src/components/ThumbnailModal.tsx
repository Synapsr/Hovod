...
/** Mounted only while open, so the picked file never survives a close. */
export function ThumbnailModal({ open, ...props }: ThumbnailModalProps) {
  if (!open) return null;
  return <ThumbnailForm {...props} />;
}

function ThumbnailForm({ onClose, asset, onSaved }: Omit<ThumbnailModalProps, 'open'>) {
  const { t } = useT();
  ...
  return (
    <Modal title={t.thumbnailModal.title} onClose={onClose} dismissible={!busy}>
