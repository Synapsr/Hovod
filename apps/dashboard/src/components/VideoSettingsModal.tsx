...
/**
 * Only the inner form holds draft state, and it is mounted exclusively while
 * the modal is open — so reopening always reseeds from the current asset
 * instead of resurrecting a stale draft.
 */
export function VideoSettingsModal({ open, ...props }: VideoSettingsModalProps) {
  if (!open) return null;
  return <VideoSettingsForm {...props} />;
}

function VideoSettingsForm({ onClose, asset, onSaved }: Omit<VideoSettingsModalProps, 'open'>) {
  const { t } = useT();
  ...
  }, [hasChanges, saving, title, description, asset, allowDownload, showTranscript, showChapters, showComments, ps, onSaved, onClose, t]);

  return (
    <Modal title={t.videoSettings.title} onClose={onClose} dismissible={!saving}>
      <div className="px-6 py-5 space-y-5">
        {/* Title */}
        <div>
          <label className="block text-xs font-medium text-zinc-400 mb-2" htmlFor="video-settings-title">
            {t.videoSettings.titleLabel}
          </label>
          <input
